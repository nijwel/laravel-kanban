...
<div class="container-fluid">
    ...
    <!-- Columns -->
    <div class="kanban-board" id="board-columns" data-board="{{ $board->id }}">
    @foreach ($board->columns as $column)
        <div class="kanban-column" data-id="{{ $column->id }}">
            <div class="d-flex justify-content-between">
                <h5 class="mb-3">{{ $column->name }} <span class="btn btn-sm add-task"  data-id="{{ $column->id }}" data-bs-toggle="modal" data-bs-target="#staticBackdrop"><img src="{{ asset('images/plus.png') }}" width="20px" alt="add"></span></h5>

                <form action="{{ route('delete.column', $column->id) }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-sm" onclick="return confirm('Are you sure you want to delete this column?')">
                        <img src="{{ asset('images/remove.png') }}" width="20px" alt="remove">
                    </button>
                </form>
                ...
                @foreach ($column->tasks->sortBy('order') as $task)
                    <div class="kanban-card task" data-id="{{ $task->id }}">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <div>
                                ...
                                <div class="card-subtitle">
                                    ({{ ucfirst($task->column?->name) }})
                                </div>
                            </div>
                            <div class="fw-bold">
                                ${{ number_format($task->order, 0, '.', ',') ?? 500 }}
                            </div>
                        </div>
                        ...
                        <div class="avatar-group d-flex mb-2">
                            ...
                            @else
                                <img src="{{ asset('images/photo.png') }}" width="20px" alt="remove">
                            @endif
                            {{-- @endforeach --}}
                        </div>
